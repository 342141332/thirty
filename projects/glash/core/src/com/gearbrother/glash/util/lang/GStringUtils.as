	...
	*/
	...
	public class GStringUtils {
		...
		*/
		...
		/**
		 * example:
		 * 	trace(StringUtils.substitute("${obj.name}: ${obj.say}", {obj: {name: "peter", say: "hello"}}));
		 * 	output: peter: hello
		 * 
		 * @param str
		 * @param replaceMap
		 ...
		 * 
		 */		
		public static function substituteDotPath(str:String, replaceMap:Object):String {
			var usdPrice:RegExp = /\$\{(.+?)\}+/g;
			var index:int = usdPrice.lastIndex;
			var r:Object;
			while (r = usdPrice.exec(str)) {
				var objPath:String = r[1];
				var map:Object = replaceMap;
				var at:int = 0;
				while (true) {
					var at2:int = objPath.indexOf(".", at);
					map = map[objPath.substring(at, at2 == -1 ? objPath.length : at2)];
					if (at2 == -1) {
						break;
					} else {
						at = ++at2;
