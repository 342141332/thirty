package com.gearbrother.glash.display {
	...
	public class GMovieBitmap extends GTransformBitmap {
		...
		protected var _definitionHandler:GPropertyPoolOperHandler;
		public function set definition(newValue:*):void {
			_definitionHandler ||= new GPropertyPoolOperHandler(null, this);
			_definitionHandler.succHandler = _handleDefinition;
			_definitionHandler.value = newValue == null ? [] : [newValue];
		}
		public function get definition():* {
			return (_definitionHandler && (_definitionHandler.value as Array).length > 0) ? _definitionHandler.value[0] : null;
		}
		protected function _handleDefinition(handler:GPropertyPoolOperHandler):void {
			var t:GMovieBitmap = this;
			if (definition is GBmdDefinition) {
				var bmdDefinition:GBmdDefinition = definition as GBmdDefinition;
				if (bmdDefinition) {
					var cacheOper:GBmdDefinition = _definitionHandler.cachedOper[definition];
					var result:GBmdMovieInfo = cacheOper.result;
			...
			} else if (definition is GFile && (definition as GFile).type == GFile.TYPE_IMAGE) {
				var file:GFile = definition as GFile;
				if (file)
					t.bitmapData = ((definition as GFile).result as Bitmap).bitmapData;
			} else {
				t.bitmapOffsets = null;
				t.bitmapDatas = null;
				t.bitmapData = null;
			}
		}
		...
		public function set bitmapDatas(newValue:Array):void {
			...
			totalFrames = _bitmapDatas ? _bitmapDatas.length : 0;
		}
