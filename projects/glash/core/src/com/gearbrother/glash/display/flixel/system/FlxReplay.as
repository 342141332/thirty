package com.gearbrother.glash.display.flixel.system {
	import com.gearbrother.glash.display.flixel.input.Keyboard;
	import com.gearbrother.glash.display.flixel.input.Mouse;
	import com.gearbrother.glash.display.flixel.system.replay.FrameRecord;
	...
	public class FlxReplay {
		...
		protected var _keys:Keyboard;
		
		protected var _mouse:Mouse;

		/**
		 * Instantiate a new replay object.  Doesn't actually do much until you call create() or load().
		 */
		public function FlxReplay(keys:Keyboard, mouse:Mouse) {
			seed = 0;
			frame = 0;
			frameCount = 0;
			finished = false;
		...
		public function recordFrame():void {
			var keysRecord:Array = _keys.record();
			var mouseRecord:MouseRecord = _mouse.record();
		...
		public function playNextFrame():void {
			...
			if (fr.keys != null)
				_keys.playback(fr.keys);
			if (fr.mouse != null)
				_mouse.playback(fr.mouse);
		}
