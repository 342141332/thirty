package {
	 ...
	 */
	public class GAlgorithmQuadTreeCollisionCase extends GMain {
		var circles:Array;
		var tree:*;
		...
		public function GAlgorithmQuadTreeCollisionCase(id:String = null) {
			super(id);
		}
...
//			tree = new GQuadtree(bounds);
			tree = new GBoxsGrid(bounds);

			initCircles();

			enableTick = true;
			stage.frameRate = 60;
			stage.addChild(new FPStatus());
		}
		...
		override public function tick(interval:int):void {
			...
			for (var i:int = 0; i < CIRCLE_COUNT; i++) {
				...
				var items:Array = tree.retrieve(rect);
//				items = circles;
