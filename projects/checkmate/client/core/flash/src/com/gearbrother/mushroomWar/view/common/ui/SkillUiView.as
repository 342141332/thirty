package com.gearbrother.mushroomWar.view.common.ui {
	...
	import com.gearbrother.mushroomWar.rpc.protocol.bussiness.SkillProtocol;
	...
	public class SkillUiView extends GNoScale implements IGDndable {
		...
		override public function handleModelChanged(events:Object = null):void {
			if (bindData is SkillModel) {
				...
				if (levelLabel)
					levelLabel.text = "lv." + (avatarModel.level.id + 1);
				/*if (levelProgress) {
					var ceil2:SkillLevelProtocol = avatarModel.levels.hasOwnProperty(avatarModel.level.id + 1) ? avatarModel.levels[avatarModel.level.id + 1] : avatarModel.levels[avatarModel.level.id];
					levelProgress.visible = true;
					levelProgress.maxValue = ceil2.exp;
					levelProgress.minValue = avatarModel.level.exp;
					levelProgress.value = avatarModel.exp;
				}*/
				var avatarView:AvatarView = new AvatarView();
				avatarView.setCartoon(avatarModel.cartoon, AvatarView.STATE_STOP_LEFT);
				addChild(avatarView);
